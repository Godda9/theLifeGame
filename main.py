...
<<<<<<< .mineimport os           # to clear console
...
def get_sosedi(array: list, i: int, j: int, width: int, height: int):
    ...
    # main diag
    if (j + 1 <= width - 1) and (i - 1 >= 0):
        if array[i - 1][j + 1] == 1:
            count += 1
            # print(f"IF 5 right+up")  # +++++++

    if j - 1 >= 0 and i + 1 <= height - 1:
        if array[i + 1][j - 1] == 1:
            count += 1
            # print(f"IF 6 left+down")   # +++++++

    # pob diag
    if (j - 1 >= 0) and (i - 1 >= 0):
        if array[i - 1][j - 1] == 1:
            count += 1
            # print(f"IF 7 left+up")  # +++++++

    if j + 1 <= width - 1 and i + 1 <= height - 1:
        if array[i + 1][j + 1] == 1:
            count += 1
            # print(f"IF 8 right+down")   # +++++++
    return count
